...
namespace B13Port.Common
{
    public static class CommonTool
    {
        ...
        public static long GenerateID(long fromCode, int wei)
        {
            ...
            for (int i = 0; i < bytetest.Length; i++) num += (bytetest[i] * i);
            int index = 0;
            int seed = wei;
        ...
        public static void TweenTextAnimation(Text text, float mOldScore, float newScore, float time, Action endAction, int pointCount = 1, bool issyb = false)
        {
            DOTween.To(delegate (float value)
            {
                text.text = ConvertNumber(value, pointCount, issyb);
        ...
        /// <returns></returns>
        public static string ConvertNumber(double number, int pointCount, bool hasSymbol = false)
        {
            string reStr;
            string symbol = "";
            if (hasSymbol)
        ...
        {
            DateTime t = DateTime.Now;//需要转为unix时间戳的时间
        ...
        #region Http
        //ASCII码从小到大排序
        public static String GetASCIIParamSrc(Dictionary<string, string> paramsMap)
        {
        ...
        //SHA1 加密
        public static string MakeSHA1(string originalText)
        {
